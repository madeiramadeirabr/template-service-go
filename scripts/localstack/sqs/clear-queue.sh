...
if [ -z "$1" ]; then
  ...
  if [ $? -eq 0 ]; then
    echo "aws --endpoint-url=http://$HOST:4566 sqs purge-queue --queue-url http://$HOST:4566/000000000000/$1"
    aws --endpoint-url=http://$HOST:4566 sqs purge-queue --queue-url http://$HOST:4566/000000000000/$1
    if [ $? -eq 0 ]; then
      echo "Queue purged"
    else
